...
import { Building2, MapPin, Calendar, Percent, FileText } from 'lucide-react';
import { formatCurrency } from '@/utility/Utility';
import { PropertyCollateralItem, DebtDetails } from '@/types/deal/Deal.sections';

interface PropertyCollateralRendererProps {
  item: PropertyCollateralItem;
  index: number;
  onDocumentPreview?: (document: any) => void;
  onDocumentDownload?: (document: any) => void;
}

export const PropertyCollateralRenderer: React.FC<PropertyCollateralRendererProps> = ({
  item,
  index,
  onDocumentPreview,
  onDocumentDownload
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        {item.buildingSize && (
          <div className="flex items-center space-x-2">
            <Building2 className="w-4 h-4 text-gray-400" />
            <div>
              <p className="text-sm font-medium text-gray-700">Building Size</p>
              <p className="text-sm text-gray-600">{item.buildingSize.toLocaleString()} sq ft</p>
            </div>
          </div>
        )}
        ...
        {item.yearBuilt && (
          <div className="flex items-center space-x-2">
            ...
            <div>
              <p className="text-sm font-medium text-gray-700">Year Built</p>
