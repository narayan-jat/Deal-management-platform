import React from "react";
import { Button } from "@/components/ui/button";
import { cn } from "@/lib/utils";
import { MatrixRoom } from "@/types/Matrix";

type ChatListProps = {
  items: MatrixRoom[];
  activeId?: string | null;
  onSelect: (id: string) => void;
  onOpenNew: () => void;
  getUnreadCount?: (roomId: string) => number;
};

const getPlural = (count: number) => count > 1 ? 's' : '';
export default function ChatList({ items, activeId, onSelect, onOpenNew, getUnreadCount }: ChatListProps) {
  return (
    <aside className="hidden md:flex md:flex-col md:w-[320px] lg:w-[360px] border-r bg-white">
      <div className="p-3 border-b flex items-center justify-between bg-white">
        <h2 className="font-semibold">Messages</h2>
        <Button size="sm" onClick={onOpenNew}>New Message</Button>
      </div>

      <div className="flex-1 overflow-y-auto">
        {items.length === 0 ? (
          <div className="p-6 text-sm text-muted-foreground text-center">
            <p>No conversations yet.</p>
            <p className="text-xs mt-1">Start a new message to begin chatting.</p>
          </div>
        ) : (
          <ul className="divide-y">
            {items.map((chat) => {
              const unreadCount = getUnreadCount ? getUnreadCount(chat.roomId) : 0;
              return (
                <li key={chat.roomId}>
                  <button
                    className={cn(
                      "w-full flex gap-3 items-start px-3 py-3 hover:bg-accent text-left transition-colors",
                      activeId === chat.roomId && "bg-accent"
                    )}
                    onClick={() => onSelect(chat.roomId)}
                  >
                    <div className="relative flex-shrink-0">
                      <img
                        src={chat.avatarUrl || `https://ui-avatars.com/api/?name=${encodeURIComponent(chat.name)}`}
                        alt={chat.name}
                        className="h-10 w-10 rounded-full object-cover"
                      />
                      {unreadCount > 0 && (
                        <span className="absolute -top-1 -right-1 bg-red-500 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center font-medium">
                          {unreadCount > 9 ? '9+' : unreadCount}
                        </span>
                      )}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between">
                        <p className="font-medium truncate text-sm">{chat.name}</p>
                        {chat.memberCount && (
                          <span className="text-xs text-muted-foreground ml-2">
                            {chat.memberCount} Member{getPlural(chat.memberCount)}
                          </span>
                        )}
                      </div>
                      {chat.topic && (
                        <p className="text-xs text-muted-foreground truncate mt-0.5">
                          {chat.topic}
                        </p>
                      )}
                      <p className="text-xs text-muted-foreground mt-1">
                        {unreadCount > 0 ? `${unreadCount} unread message${unreadCount > 1 ? 's' : ''}` : ''}
                      </p>
                    </div>
                  </button>
                </li>
              );
            })}
          </ul>
        )}
      </div>
    </aside>
  );
} 